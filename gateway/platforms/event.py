...
class MessageEvent:
    ...
    # Process-local completion waiters for durable plugin ingress. These stay out of metadata
    # because metadata may be logged or persisted.
    _processing_completion_futures: List["asyncio.Future[ProcessingOutcome]"] = field(
        default_factory=list, repr=False, compare=False
    )
    _plugin_subprocess_env: Dict[str, str] = field(default_factory=dict, repr=False, compare=False)
...
def resolve_processing_completion(event: MessageEvent, outcome: ProcessingOutcome) -> None:
    """Resolve and clear every process-local completion waiter on *event*."""
    futures = list(event._processing_completion_futures)
    event._processing_completion_futures.clear()
    for future in futures:
        if not future.done():
            future.set_result(outcome)


def merge_processing_completion_waiters(target: MessageEvent, incoming: MessageEvent) -> None:
    """Transfer delivery ownership when two pending events are coalesced."""
    if incoming._processing_completion_futures:
        target._processing_completion_futures.extend(incoming._processing_completion_futures)
        incoming._processing_completion_futures.clear()
