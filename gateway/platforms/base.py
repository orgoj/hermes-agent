...
from gateway.platforms.event import (
    ...
    MessageType,
    ProcessingOutcome,
    merge_processing_completion_waiters as _merge_processing_completion_waiters,
    resolve_processing_completion as _resolve_processing_completion,
)
...
class BasePlatformAdapter(ABC):
    ...
    def _start_session_processing(self, event: MessageEvent, session_key: str, *,
        ...
        task = asyncio.create_task(self._process_message_with_event_context(event, session_key))
    ...
    async def _process_message_with_event_context(
        ...
        """Bind task-local plugin environment values for the whole delivery-owned turn."""
        if not event._plugin_subprocess_env:
            await self._process_message_background(event, session_key)
            return
        from gateway.plugin_context import scoped_subprocess_env

        with scoped_subprocess_env(event._plugin_subprocess_env):
            await self._process_message_background(event, session_key)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            _resolve_processing_completion(
                event, ProcessingOutcome.SUCCESS if processing_ok else ProcessingOutcome.FAILURE)
            ...
            _resolve_processing_completion(
                event, ProcessingOutcome.CANCELLED if expected else ProcessingOutcome.FAILURE)
            raise
    ...
    def _spawn_drain_task(self, pending_event: MessageEvent, session_key: str) -> None:
        ...
        self._track_session_task(
            session_key,
            asyncio.create_task(self._process_message_with_event_context(pending_event, session_key)))
