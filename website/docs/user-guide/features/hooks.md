...
| `on_cli_ready` | Observer | Interactive CLI is ready immediately before its input loop starts. | `cli_surface="interactive"` | Surface state only. |
| `on_cli_turn_start` | Observer | Interactive CLI accepted input and is about to run the agent turn. | `cli_surface="interactive"` | Surface state only. |
| `on_cli_turn_end` | Observer | Interactive CLI finished turn cleanup and is returning to input. | `cli_surface="interactive"` | Surface state only. |
| `on_cli_shutdown` | Observer | Interactive CLI input loop has stopped, before final resource cleanup. | `cli_surface="interactive"` | Surface state only. |
