...
def _inject_session_context_env(env: dict) -> None:
    ...
    # hcom bridge values are task-local for the same reason as session
    # identity: a multi-session gateway must never leak one route's catalog
    # namespace into a sibling terminal command.
    try:
        from gateway.hcom_bridge import subprocess_env_values

        hcom_values = subprocess_env_values()
    except Exception:
        hcom_values = {}
    for name in ("HCOM_DIR", "HCOM_AGENT_CATALOGS"):
        if name in hcom_values:
            if hcom_values[name]:
                env[name] = hcom_values[name]
            else:
                env.pop(name, None)
